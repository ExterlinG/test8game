...
void Planet::Init() 
{
	...
	for (int i = 0; i < PLANET_MAX; i++) 
	{
		...
		planets[i] = Planet(
			...
			defaultPos[i].x,    // ÑAÑuÑÇÑuÑ} X ÑyÑx ÑÄÑqÑãÑuÑsÑÄ Ñ}ÑpÑÉÑÉÑyÑrÑp
			defaultPos[i].y,    // ÑAÑuÑÇÑuÑ} Y ÑyÑx ÑÄÑqÑãÑuÑsÑÄ Ñ}ÑpÑÉÑÉÑyÑrÑp
			PLANET_CENTER,          // ÑQÑpÑtÑyÑÖÑÉ
			Planet::Owner::NEUTRAL,
			200,
			groundImage       // ÑHÑpÑsÑÇÑÖÑwÑuÑ~Ñ~ÑpÑë ÑÑÑuÑ{ÑÉÑÑÑÖÑÇÑp
		);
	}
	//----------------------------------------------
		//angle
